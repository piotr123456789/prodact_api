...
<container class="color-bgfull">
   ...
   <input type="text" class="form-control input-control" v-model="slug" name="filed" v-validate="'required|max:130|alpha'" id="exampleFormControlInput1" placeholder="np: białogóra">
...
export default {
  ...
  methods:{
    ...
    submit(e){
      ...
      axios.post('http://localhost:3000/postText',data)
        .then(res => {this.checkValidServer = res.data;})
        .catch(err => console.log('error'+err));
      console.log('wykonalo sie');
    },
    formvalid(e){
